...
#                       CONSTRUCTOR TESTS
# ------------------------------------------------------------------
def test_reverts_if_token_length_are_different(
...
def test_can_deposit_collateral_without_minting(dsc, some_user):
...
def test_reverts_if_minted_dsc_breaks_health_factor(dsce, weth, eth_usd, some_user):
    ...
    with boa.env.prank(some_user):
        ...
        with boa.reverts("DSCEngine: Health factor is broken"):
...
def test_can_redeem_collateral(dsce, weth, dsc, some_user):
    with boa.env.prank(some_user):
        ...
        dsce.deposit_and_mint(weth, COLLATERAL_AMOUNT, AMOUNT_TO_MINT)
...
def test_must_improve_health_factor_on_liquidation(
    ...
    with boa.env.prank(liquidator):
        ...
        dsce.deposit_and_mint(weth, collateral_to_cover, AMOUNT_TO_MINT)
        ...
        with boa.reverts("DSCEngine: Didn't improve health factor"):
...
def test_liquidation_payout_is_correct(
    dsce_liquidated, weth, liquidator
):
...
def test_get_collateral_token_price_feed(dsce, weth, eth_usd):
    price_feed = dsce.token_to_price_feed(weth)
